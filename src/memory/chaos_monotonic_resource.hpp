...
#define REGISTER_MEMORY_STAT(name) REGISTER_CHAOS_STAT(name)
#define CHAOS_MEM_RECORD(name, val) CHAOS_MEM_RECORD(name, val)
#endif

DEFINE_MEMORY_STAT(MemoryCapacity, "MemoryCapacity", SC::ChaosThroughput<>,
                   false);
REGISTER_MEMORY_STAT(MemoryCapacity)

DEFINE_MEMORY_STAT(MemoryBlocks, "MemoryBlocks",
                   SC::ChaosThroughput<SC::MetricUnits>, false);
REGISTER_MEMORY_STAT(MemoryBlocks)

DEFINE_MEMORY_STAT(MemoryRequested, "MemoryRequested", SC::ChaosThroughput<>,
                   false);
...
DEFINE_MEMORY_STAT(AllocationCount, "AllocationCount",
                   SC::ChaosThroughput<SC::MetricUnits>, false);
REGISTER_MEMORY_STAT(AllocationCount)

DEFINE_MEMORY_STAT(ContentionCount, "ContentionCount",
                   SC::ChaosThroughput<SC::MetricUnits>, false);
REGISTER_MEMORY_STAT(ContentionCount)

DEFINE_MEMORY_STAT(BlockOverflowCount, "BlockOverflowCount",
                   SC::ChaosThroughput<SC::MetricUnits>, false);
REGISTER_MEMORY_STAT(BlockOverflowCount)

DEFINE_MEMORY_STAT(CustomBlocksCount, "CustomBlocks",
                   SC::ChaosThroughput<SC::MetricUnits>, false);
    ...
    while (true) {
      ...
      if (next.current <= block.end) {
        if (m_range.compare_exchange_weak(block, next,
          ...
          CHAOS_MEM_RECORD(MemoryWaste,
                           static_cast<uint64_t>(aligned_addr - curr_addr))
#ifndef DISABLE_CHAOS_MEMORY_STATS
          waste.fetch_add(static_cast<uint64_t>(aligned_addr - curr_addr),
