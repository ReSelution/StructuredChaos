...
namespace sc {
  ...
  public:
    explicit ChaosArena(size_t size = 8 * 1024) :
        m_capacity(size), m_backingBuffer(std::make_unique_for_overwrite<uint8_t[]>(m_capacity)),
    ...
    T *make(Args &&...args) {
      ...
      if constexpr (!std::is_trivially_destructible_v<T>) {
        ...
        auto *newNode = new (nodeMem)
            CleanupNode{.destroyer = [](void *p) { static_cast<T *>(p)->~T(); }, .object = obj, .next = nullptr};

        newNode->next = m_cleanupHead.load(std::memory_order_relaxed);
        while (!m_cleanupHead.compare_exchange_weak(newNode->next, newNode, std::memory_order_release,
    ...
    [[nodiscard]] void *allocate(size_t size, size_t align) { return m_pool.allocate(size, align); }
