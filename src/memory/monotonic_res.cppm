...
#define DISABLE_CHAOS_MEMORY_STATS
  ...
  // ============================================================================
  export using StatCapacity     = sc::stats::Stat<"MemoryCapacity", sc::stats::Throughput<>>;
  export using StatBlocks       = sc::stats::Stat<"MemoryBlocks", sc::stats::Throughput<sc::stats::MetricUnits>>;
  export using StatRequested    = sc::stats::Stat<"MemoryRequested", sc::stats::Throughput<>>;
  export using StatWaste        = sc::stats::Stat<"MemoryWaste", sc::stats::Throughput<>>;
  export using StatAllocCount   = sc::stats::Stat<"AllocationCount", sc::stats::Throughput<sc::stats::MetricUnits>>;
  export using StatContention   = sc::stats::Stat<"ContentionCount", sc::stats::Throughput<sc::stats::MetricUnits>>;
  export using StatOverflow     = sc::stats::Stat<"BlockOverflowCount", sc::stats::Throughput<sc::stats::MetricUnits>>;
  export using StatCustomBlocks = sc::stats::Stat<"CustomBlocks", sc::stats::Throughput<sc::stats::MetricUnits>>;
...
#define CHAOS_MEM_RECORD(StatType, val) StatType::record(val);
  ...
  public:
    ...
    explicit ChaosMonotonicResource(void *buffer, size_t size,
                                    std::pmr::memory_resource *upstream = std::pmr::get_default_resource()) :
        m_upstream(upstream), m_initial_block(static_cast<std::byte *>(buffer), size), m_next_block_size(size * 2) {
    ...
    void release() noexcept {
      ...
      if (m_initial_block.ptr) {
        CHAOS_MEM_RECORD(StatCapacity, -static_cast<int64_t>(m_initial_block.size))
      }
    ...
    void *do_allocate(size_t bytes, size_t alignment) final {
      ...
      while (true) {
        ...
        if (next.current <= block.end) {
          if (m_range.compare_exchange_weak(block, next, std::memory_order_acq_rel)) {
            uint64_t alignment_waste = static_cast<uint64_t>(aligned_addr - curr_addr);

            CHAOS_MEM_RECORD(StatRequested, bytes)
            CHAOS_MEM_RECORD(StatWaste, alignment_waste)

#ifndef DISABLE_CHAOS_MEMORY_STATS
            m_waste_accumulator.fetch_add(alignment_waste, std::memory_order_relaxed);
#endif
            return aligned_ptr;
          }
