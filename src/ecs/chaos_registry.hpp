...
// } && std::is_same_v<typename get_class_from_member<decltype(Method)>::type,
// entt::registry>;
...
DEFINE_CHAOS_STAT(Entities, "Entities", SC::ChaosThroughput<SC::MetricUnits>,
                  false);
...
public:
  ...
  template <typename... Components> decltype(auto) view();
...
template <typename... Components> decltype(auto) ChaosRegistry::view() {
  return executeRead<Components...>(
      [&]() -> decltype(auto) { return m_reg.view<Components...>(); });
}

template <typename Component> auto *ChaosRegistry::getComponentAccess() {
...
template <typename Component> auto *ChaosRegistry::getComponentAccess() const {
  using AccessType = ComponentAccess<Component>;
...
template <typename Component> void ChaosRegistry::connectOnDestroy() {
  ...
  if constexpr (has_resource) {
    m_reg.on_destroy<Component>()
        .template connect<&ChaosRegistry::cleanupResources<Component>>();
  }
}
...
template <typename... Components> struct [[nodiscard]] ChaosLock {
  std::tuple<AlwaysLock<Components>...> locks;

  ChaosLock(std::tuple<AlwaysLock<Components>...> &&l) : locks(std::move(l)) {
    lock();
  }
...
decltype(auto) ChaosRegistry::executeRead(Func &&func) {
  ...
  ChaosLock<Components...> lock{std::make_tuple(std::shared_lock{
...
template <typename T> decltype(auto) ChaosEntity::get() const {
