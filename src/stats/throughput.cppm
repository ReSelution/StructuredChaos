...
export module sc.stats:throughput;
import :core;
import :units;

import sc.util; // Für FixedString, falls benötigt
...
namespace sc::stats {
  ...
  export template<typename UnitSystem = DataUnits>
  struct Throughput {
    ...
    static std::string format(const Storage &s) {
      ...
      if (s.running.load(std::memory_order_acquire)) {
        auto startTicks = s.startTimeTicks.load(std::memory_order_acquire);
        auto startPoint =
            std::chrono::high_resolution_clock::time_point{std::chrono::high_resolution_clock::duration{startTicks}};
