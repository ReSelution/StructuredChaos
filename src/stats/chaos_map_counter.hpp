...
namespace SC {
    ...
    struct ChaosMapCounter {
        ...
        struct alignas(64) Storage {
            ...
            using denseMap = ankerl::unordered_dense::map<Key, uint64_t, IdentityHash>;
            denseMap counts;
        ...
        static std::string format(const Storage &s) {
            ...
            for (size_t i = 0; i < limit; ++i) {
                details += std::format("    {}.\t-> {} ({}){}", i + 1, sorted[i].first, sorted[i].second,
                                       (i < limit - 1 ? ",\n" : ""));
            }
            return std::format("Unique: {} | Total: {} | Top: {} ({}) \n{}", s.counts.size(), total, sorted[0].first,
                               sorted[0].second, details);
        }
    };
