...
export template <typename Func> class ChaosTimer {
public:
  [[nodiscard]] explicit ChaosTimer(Func &&callback,
                                    const Unit unit = Unit::Auto)
  ...
  ChaosTimer(const ChaosTimer &) = delete;
  ChaosTimer &operator=(const ChaosTimer &) = delete;
...
ChaosTimer(Func &&, Unit = Unit::Auto) -> ChaosTimer<std::decay_t<Func>>;
