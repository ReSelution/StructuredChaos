...
using aQueue = atomic_queue::AtomicQueueB2<MoveOnlyFunction,
                                           std::allocator<MoveOnlyFunction>>;
...
struct alignas(64) Queues {
  ...
  bool try_pop_any(MoveOnlyFunction &task) {
    return try_pop_impl(task, std::make_index_sequence<static_cast<size_t>(
                                  Priority::PriorityCount)>{});
  }

  [[nodiscard]] bool was_empty() const {
    return was_empty_impl(std::make_index_sequence<static_cast<size_t>(
                              Priority::PriorityCount)>{});
  }
...
constexpr size_t MAX_TASKS =
    QUEUE_CAP * static_cast<int32_t>(ChaosThreading::Priority::PriorityCount);
...
size_t ChaosThreading::getNumThreads() {
  return threads.size() + 1 +
         longRunningThreads.size(); // +1 to include main Thread
}

int32_t ChaosThreading::getThreadId() { return threadId; }
...
template void ChaosThreading::pushTask<ChaosThreading::Priority::High>(
    MoveOnlyFunction &&task);

template void ChaosThreading::pushTask<ChaosThreading::Priority::Normal>(
    MoveOnlyFunction &&task);

template void ChaosThreading::pushTask<ChaosThreading::Priority::Low>(
    MoveOnlyFunction &&task);
...
template void ChaosThreading::pushBatch<ChaosThreading::Priority::High>(
    std::vector<MoveOnlyFunction> &&);

template void ChaosThreading::pushBatch<ChaosThreading::Priority::Normal>(
    std::vector<MoveOnlyFunction> &&);

template void ChaosThreading::pushBatch<ChaosThreading::Priority::Low>(
    std::vector<MoveOnlyFunction> &&);

void ChaosThreading::pushLongTaskInternal(
    std::string_view name, std::function<void(std::stop_token)> task) {
  if (!threads.empty()) {
  ...
  std::lock_guard lock(threadMutex);
  std::erase_if(longRunningThreads,
                [](const std::jthread &t) { return !t.joinable(); });

  longRunningThreads.emplace_back(
      [name, task = std::move(task)](const std::stop_token &st) {
...
void ChaosThreading::wait_until_finished() {
  ...
  wait_cv.wait(lock, [] {
    return QueueSize::m_storage.value.load() == 0 &&
           ActiveTask::m_storage.value.load() == 0;
  });
}
